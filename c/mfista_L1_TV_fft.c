...
void usage(char *s)
{
  printf("%s <fft_data fname> <double lambda_l1> <double lambda_tv> <double c> <X outfile> {X initfile} {-nonneg} {-fftw_measure} {-log log_fname}\n\n",s);
  ...
  printf(" c is a parameter used for stepsize. Large c makes the algorithm stable \n");
  printf(" but slow. Around 500000 is fine.\n\n");
...
int main(int argc, char *argv[]){
  ...
  double *y_r, *y_i, *noise_stdev, *xvec, 
    *mask_h, *mask, cinit = CINIT, lambda_l1, lambda_tv, s_t, e_t;
  ...
  calc_result_fft(yf, mask_h, M, &NN, NX, NY,
		  lambda_l1, lambda_tv, 0, xvec, &mfista_result);
