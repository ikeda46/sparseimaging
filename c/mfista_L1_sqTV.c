...
void usage(char *s)
{
  printf("%s <int m> <intl n> <V fname> <A fname> <double lambda_l1> <double lambda_sqtv> <double c> <X outfile> {X initfile} {-t} {-rec NX} {-looe} {-nonneg} {-log log_fname}\n\n",s);
  ...
  printf("  <double lambda_sqtv>: value of lambda_sqtv. Positive.\n");
  ...
  printf(" argmin |v-Ax|_2^2/2 + lambda_l1 |x|_1 + lambda_sqtv sqTV(x)\n\n");
...
{
  double *y, *A, *xvec, cinit = CINIT, lambda_l1, lambda_sqtv;
  ...
  lambda_sqtv = atof(argv[6]);
  printf("lambda_sqtv = %g\n",lambda_sqtv);
  ...
  mfista_L1_sqTV_core(y, A, &M, &N, NX, NY,
		      lambda_l1, lambda_sqtv, cinit, xvec, nonneg_flag, looe_flag,
		      &mfista_result);
  ...
  mfista_result.out_fname = argv[8];  
