function [x,cost,LOOE] = MFISTA_L1_TV(y,A,Nx,Ny,xinit,lambda_1,lambda_tv,iso_type,cinit)
% function [x,cost,LOOE] = MFISTA_L1_TV(y,A,Nx,Ny,xinit,lambda_1,lambda_tv,iso_type,cinit)
%
%    y: observed vector
%    A: matrix
...
%
%    min_x (1/2)||y-A*x||_2^2 + lambda*sum(abs(x)) + lambda_tv*TV(x)
%    
...
tmpc = (y-A*x)'*(y-A*x)/2+lambda_1*sum(abs(x))+lambda_tv*TVcost;
...
for t = 1:MAXITER
    ...
    for i = 1:MAXITER
        ...
        xtmp = FGP_L1(AyAz/c+z,lambda_1/c,lambda_tv/c,Nx,Ny,ITER,flag_iso);
        yAxtmp = y-A*xtmp;
        
        tmpF = yAxtmp'*yAxtmp/2;
        tmpQ = coreQ - (xtmp-z)'*AyAz + (xtmp-z)'*(xtmp-z)*c/2; 
...
%    fprintf('Total variation is %g\n',TVcost);
    ...
    if tmpF + lambda_1*L1cost + lambda_tv*TVcost < tmpcost(t)
        tmpc = tmpF + lambda_1*L1cost + lambda_tv*TVcost;
